...
1. **Lexicon scan** (`lexicon.json`) — deterministic regex detection of banned patterns, split into *hard* (unambiguous slop: "delve", "I hope this helps", "studies show" without a source…) and *soft* (weaker or context-legitimate signals, reported but not release-blocking: "leverage" in finance, "robust" in engineering, and em-dash density — a real but weak statistical tell per the research). Structural scans catch decorative bold-colon listicles (short vague label-fragments, not section headings or spec values) and decorative emoji. A banned phrase *quoted on a prohibition line* ("Do not use 'I understand how you feel'") is a mention, not a use — the model teaching avoidance — so quoted spans on negation-cue lines are excluded before matching; unquoted slop, even on a negation line, is still caught.
2. **Blind pairwise judging** — an LLM judge scores both outputs on density, directness, honesty, and structure, and picks a winner without knowing which output used SuperMD. Order is randomized per scenario. Purely subjective creative-copy tasks (landing pages, menu copy) are marked `noJudge`: they keep the deterministic lexicon scan but skip the pairwise judge, because LLM judges are documented to be length- and punch-biased and will rate concise, honest copy as "thin" or prefer a punchier version that carries an inflated stat — an aesthetic verdict the objective scan measures better than the judge.
...
- pairwise win rate ≥ 80%, and no scenario lost to baseline — with two guards against judge unreliability: (1) a lone "baseline" verdict triggers a best-of-3 tiebreak (two more independent generate-and-judge rounds), counting only if baseline wins the majority; (2) a confirmed baseline win is still downgraded to a transparent *note* rather than a failure in two cases: when the deterministic lexicon scan contradicts it (SuperMD has fewer hard+soft hits than baseline), or when the judge's own reason *concedes* the baseline it preferred was inflated, fabricated, or unsupported ("wins despite an inflated 30% claim") — rewarding acknowledged slop over SuperMD's honest output is the judge's failure, not SuperMD's, and honesty outranks the punch the judge fell for. Both because LLM judges reward fluent slop, especially in marketing copy and Indonesian, which is exactly why the objective scan exists alongside them
